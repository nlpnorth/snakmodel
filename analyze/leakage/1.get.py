...
def save(dataset, name):
    if os.path.isfile(name):
        ...
        return
    print('Saving ' + name + '...')
    if '/' in name and not os.path.isdir(name.split('/')[0]):
...
da_datasets = ['alexandrainst/danish-citizen-tests', 'chcaa/dansk-ner', 'alexandrainst/nordjylland-news-summarization', 'alexandrainst/danish-citizen-tests', 'DDSC/angry-tweets']
for dataset in da_datasets:
    convert(dataset)
da_datasets = ['alexandrainst/m_hellaswag', 'alexandrainst/scandi-qa', 'alexandrainst/scala']
for dataset in da_datasets:
    convert(dataset, 'da')
